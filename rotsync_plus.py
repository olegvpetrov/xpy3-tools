""" 
Same as rotsync but employs advanced echo quantitation aiming at noise reduction. 
To ensure that such quantitation does not lead to errors, it is recommended to use 
rotsync_plus in conjuction with rotsync.

This script picks up midpoints of MAS rotory echoes from an arbitrarily sampled
fid, save them as a new fid signal and processes with the command efp. The resulting
spectrum is analogous to ones from rotor-synchronized acquisition experiments.

The rotor-synchronized spectrum has no spinning sidebands and its centerband consists 
of central and satellite transitions on a par [see J. Magn. Reson. 177 (2005) 44-55)]. 
It can be useful for quantitation of species with distinct chemical shifts through  
centerband integration or for linesape analysis in the limit of an infinite MAS rate
(applies to spins-3/2, 5/2, etc.). 

The resulting spectrum is output to a new Topspin window, with EXPNO set next to
the maximum EXPNO found under the given NAME. An auxiliary window pops up to control 
echo pattern recognition and, if need be, to adjust the echo positions manually.
...
def get_baseline(y, mask, l=None): 
    w = np.asarray(mask, dtype='d')  
    if not l:  
       # run version with V-curve optimization of lambda
        lrange = array.array('d',np.linspace(1,11,101))
        z, l = ws2doptv(y, w, lrange)
        return np.asarray(z), np.log10(l)
    else: 
       # run basic version
        z = ws2d(y, 10**l, w)
        return np.asarray(z) 
...
def get_sb_period(data, cb) -> int:
    ...
    :return: spinning sideband period (as number of points)
    '''
...
# get the dataset as a brukerIO.dataset object for extended I/O capabilities
...
# estimate centerband limits (index range)
...
# switch to a new EXPNO removing the digital filter part from fid
...
# raise the window and await user commands
root = tk.Tk()
...
tk.Label(controls, text="offset").grid(row=1, column=0, padx=(25,5), sticky='e')
tk.Label(controls, textvariable=offset, width=5).grid(row=1, column=2, sticky='w')
tk.Label(controls, text="period").grid(row=2, column=0, padx=5, sticky='e')
...
if ok_flag:
    ...
    ''' --------------- echo quantitation block --------------- '''

    # apply a lowpass filter to fid:
    cutoff = len(fid) / period  
    filt_fid = butter_lowpass_filter(fid[::-1], cutoff, len(fid))[::-1]

    # interpolate fid between the echoes' bases:  
    mask = np.full(len(fid), False)
    mask[samples] = True
    mask = np.roll(mask, -int(period / 2))

    base_real, _ = get_baseline(filt_fid.real, mask)
    base_imag, _ = get_baseline(filt_fid.imag, mask)
    base = base_real + 1j*base_imag

    # extract the echo component from fid:
    fid_minus = fid - base

    # quantify the echo component through PCA
    s = int(samples[0] / 2)
    s = s // zoom * zoom 
    echo_segments = [fid_minus[i:j+1] for i,j in zip(samples-s, samples+s)]

    (a := echo_segments[-1] + 0).resize(2*s + 1)
    echo_segments[-1] = a

    pca = PCA( np.asarray(echo_segments)[:][:,::zoom] )
    npc = pca.malinowski_indicator()

    echo_heights = pca.evaluate_at_index(int(s/zoom), n=npc)
    echo_heights += base[samples]

    # prepend fid's first point to the echoes:
    echo_heights = np.r_[fid[0], echo_heights]

    ''' ------------------------------------------------------- '''
    
    # replace the original fid with the echo sample 
    dta.writefidc(echo_heights)
    ...
    # NMRDataSet.setPar() does not work with status parameters, hence the use of brukerIO
    ...
    # delete newly created EXPNO, return to original EXPNO
    proton.launch('close')
